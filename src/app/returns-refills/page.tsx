    ...
    <div className="grid gap-6">
      ...
      <section className="grid gap-4">
        {cylinders.map((cylinder) => (
          <article key={cylinder.id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-soft">
            <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
              <div>
                <h2 className="text-lg font-bold">{cylinder.cylinderId}</h2>
                <p className="text-sm text-slate-600">{cylinder.brand} {cylinder.size} • {cylinder.serialNumber}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 self-end md:grid-cols-4">
                {actions.map((action) => {
                  ...
                  return (
                    <button
                      key={action.label}
                      className="flex min-h-10 items-center justify-center gap-2 rounded-md border border-slate-200 px-3 py-2 text-xs font-bold text-charcoal hover:bg-slate-50"
